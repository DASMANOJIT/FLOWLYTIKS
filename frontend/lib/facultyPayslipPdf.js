...
const drawFooter = (doc, startY) => {
  ...
  const footerY = Math.min(pageHeight - 24, startY);
...
const drawReceiptShell = ({ doc, logoImage, title, subtitle, meta, amount, status, fontName }) => {
  ...
  addGradientAccent(doc);

  doc.setFillColor(255, 255, 255);
  doc.rect(0, 10, pageWidth, pageHeight - 10, "F");
...
const drawInfoTable = ({ doc, startY, title, rows, fillColor }) => {
  autoTable(doc, {
    ...
    alternateRowStyles: {
      fillColor: fillColor === PRIMARY ? PANEL_BG : [252, 252, 253],
    },
    columnStyles: {
...
const normalizeDetailsRows = (payroll, currencyFallback) => {
  ...
  return details.map((row) => [
    ...
    currencyText(row.amount, { fallback: currencyFallback }),
...
const compactRows = (rows) => rows.filter(([, value]) => safe(value) !== "-");
...
export const downloadFacultyPayslipPdf = async ({ profile, payroll }) => {
  ...
  const weekPeriod = `${dateText(payroll.weekStart)} to ${dateText(payroll.weekEnd)}`;
  const paymentStatus = payroll.paymentStatus || payroll.status || "PENDING";
  const totalEntries = payroll.totalEntries ?? payroll.attendanceEntries ?? payroll.presentDays ?? payroll.presentEntries ?? 0;
  const paymentMode = payroll.paymentMode || payroll.paymentMethod || payroll.payoutMode || "-";
  const transactionId = payroll.utr || payroll.transactionId || payroll.cashfreeTransactionId;
  const transferReference =
    payroll.cashfreeReferenceId || payroll.gatewayReference || payroll.referenceId || payroll.cashfreeTransferId;
  ...
  drawInfoTable({
    ...
    rows: [
      ...
      ["Payment Mode", statusLabel(paymentMode)],
      ["Payment Status", statusLabel(paymentStatus)],
      ["Total Attendance Entries", safe(totalEntries, "0")],
      ...
      ["Bonus", currencyText(payroll.bonus || 0, { fallback: currencyFallback })],
      ["Deduction", currencyText(payroll.deduction || 0, { fallback: currencyFallback })],
      ...
      ["UTR / Transaction ID", safe(transactionId)],
      ["Cashfree Transfer / Reference ID", safe(transferReference)],
      ["Bank Account", maskedAccountText(payroll.accountNumber || payroll.bankAccountNumber)],
      ["Remarks", safe(payroll.remarks || payroll.failureReason)],
    ],
  });

  const detailRows = normalizeDetailsRows(payroll, currencyFallback);
  ...
  drawFooter(doc, (doc.lastAutoTable?.finalY || 230) + 14);
