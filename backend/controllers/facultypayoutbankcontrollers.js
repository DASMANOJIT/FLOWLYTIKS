...
const normalizeAdminBankData = (body, actor) => {
  ...
  return {
    ...
    verificationStatus,
    payoutEligible: verificationStatus === "VERIFIED" ? Boolean(body.payoutEligible) : false,
...
export const createBankAccount = async (req, res) => {
  try {
    ...
    return res.status(201).json({ success: true, bankAccount: serializeBankAccount(record, { includeFullAccount: true }) });
  } catch (err) {
    console.error("Create bank account error:", err?.message || err);
    return res.status(500).json({ success: false, message: "Failed to create bank account." });
  }
};
...
export const updateBankAccount = async (req, res) => {
  try {
    ...
    const data = normalizeAdminBankData(req.body, `admin:${req.user?.id || "unknown"}`);
    const record = await payoutService.updateFacultyBankAccount(id, data);
    return res.json({ success: true, bankAccount: serializeBankAccount(record, { includeFullAccount: true }) });
  } catch (err) {
...
export const getBankAccountByFaculty = async (req, res) => {
  try {
    ...
    if (req.userRole === "faculty" && String(facultyId) !== String(req.user?.id)) {
      return res.status(403).json({ success: false, message: "Faculty members can only view their own bank details." });
    }
    const account = await payoutService.getFacultyBankAccount(facultyId);
    return res.json({ success: true, bankAccount: serializeBankAccount(account, { includeFullAccount: req.userRole === "admin" || String(facultyId) === String(req.user?.id) }) });
  } catch (err) {
    console.error("Get bank account error:", err?.message || err);
    return res.status(500).json({ success: false, message: "Failed to get bank account." });
  }
};
...
export const verifyBankAccount = async (req, res) => {
  try {
    ...
    return res.json({ success: true, bankAccount: serializeBankAccount(account, { includeFullAccount: true }) });
  } catch (err) {
...
export const updateMyBankAccount = async (req, res) => {
  try {
    ...
    const record = await payoutService.createFacultyBankAccount(normalizeFacultyBankData(req.body, req.user.id));
    return res.json({
      success: true,
