...
import { sendFacultyWeeklyPayoutEmail } from "../services/emailNotificationService.js";
...
const sendWeeklyPayoutEmailsForRecord = async (recordId) => {
  ...
  await Promise.allSettled(
    (record.facultyRecords || [])
      .filter((row) => row.status === "PAID")
      .map((row) =>
        sendFacultyWeeklyPayoutEmail({
          faculty: row.faculty,
          payout: row,
          idempotencyKey: `faculty-weekly-payout-paid:${record.id}:${row.facultyId}`,
          breakdown: {
            weekStart: record.weekStart,
            weekEnd: record.weekEnd,
            paymentMethod: record.paymentMode === "CASH" ? "Cash" : "Cashfree Payout",
            payableAmount: row.amount,
            paidAmount: row.amount,
            paidAt: row.paidAt || record.paidAt,
            reference: row.utr || row.transactionId || row.cashfreeReferenceId || row.cashfreeTransferId || "-",
            payrollId: record.payrollCycleId,
          },
        })
      )
  );
...
export const payWeeklyCash = async (req, res) => {
  try {
    ...
    await sendWeeklyPayoutEmailsForRecord(record.id);
